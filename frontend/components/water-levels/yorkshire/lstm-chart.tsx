...
export function YorkshireLSTMChart() {
    ...
    const fetchData = async () => {
      try {
        const [histRes, forecastRes, accRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/water-levels/yorkshire-prediction-accuracy/?model_type=LSTM`),
        ]);
    ...
    <Card className="shadow-lg border-0">
      <CardHeader>
        <div className="flex items-center justify-between space-y-4 sm:space-y-0">
          <CardTitle className="text-xl font-bold">Yorkshire Forecast - LSTM</CardTitle>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-800">
                Forecast generated using an LSTM model trained on historical Yorkshire data.
              </p>
            </div>
          </div>
        </div>
