    ...
    fetch(`${API_BASE}/api/water-levels/yorkshire/reservoir-data/`)
      ...
      .then((d: Entry[]) => {
        if (Array.isArray(d) && d.length > 0) {
          const latest = d[0]
          const prev = d[1] ?? d[0]
          const avg = d.reduce((a, b) => a + b.reservoir_level, 0) / d.length
          const change = +(latest.reservoir_level - prev.reservoir_level).toFixed(2)
          const diffAvg = +(latest.reservoir_level - avg).toFixed(2)
          setCurrentData({
            currentLevel: latest.reservoir_level,
            lastUpdated: latest.report_date,
          ...
          const trendValues = generateTrend(latest.reservoir_level, change)
      ...
      <Card className="lg:col-span-2 shadow-lg border-0">
        <CardHeader className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-bold">Yorkshire Reservoir Level</CardTitle>
      ...
      <Card className="shadow-lg border-0">
        ...
        <CardContent>
          ...
          {currentData && currentData.currentLevel < 70 && (
            <div className="mt-4 p-3 bg-orange-50 dark:bg-orange-900 border border-orange-200 rounded-lg">
