...
export function ScottishRegionalLSTMChart({ area }: { area: string }) {
    ...
    const fetchData = async () => {
      try {
        const [histRes, forecastRes, accRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/water-levels/scottishwater/regional/${area}/LSTM/`),
          fetch(
    ...
    <Card className="shadow-lg border-0">
      <CardHeader>
        <div className="flex items-center justify-between space-y-4 sm:space-y-0">
          <CardTitle className="text-xl font-bold">Scottish Water Regional Forecast - LSTM</CardTitle>
