...
export function ScottishRegionalRegressionChart({ area }: { area: string }) {
    ...
    const fetchData = async () => {
      try {
        const [histRes, forecastRes, accRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/water-levels/scottishwater/regional/${area}/REGRESSION/`),
          fetch(
            `${API_BASE}/api/water-levels/scottishwater-prediction-accuracy/?area=${area}&model_type=REGRESSION`
          ),
        ]);
        ...
        if (
          ...
          // Limit to last 4 forecasts (same as other charts)
    ...
    <Card className="shadow-lg border-0">
      <CardHeader>
        <div className="flex items-center justify-between space-y-4 sm:space-y-0">
          <CardTitle className="text-xl font-bold">Scottish Water Regional Forecast - REGRESSION</CardTitle>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-800">
                Forecast generated using a REGRESSION model trained on historical Scottish Water regional data.
              </p>
            </div>
          </div>
        </div>
