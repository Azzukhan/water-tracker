...
export function ScottishRegionalHistoryChart({ area }: { area: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/water-levels/scottish-regions?area=${area}`);
        ...
        if (Array.isArray(json)) {
          ...
          const avg =
            sorted.reduce((sum, e) => sum + e.current, 0) / sorted.length;
        ...
        <div className="h-80 mb-6">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              />
              <ReferenceLine
                y={allData.length ? allData[0].average : 0}
                stroke="#6b7280"
                strokeDasharray="5 5"
              ...
              <Line type="monotone" dataKey="average" stroke="#6b7280" strokeWidth={2} strokeDasharray="5 5" dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
