...
export function ScottishRegressionChart() {
    ...
    const fetchData = async () => {
      try {
        const [histRes, forecastRes, accRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/water-levels/scottishwater-forecast-accuracy/?model_type=REGRESSION`)
        ]);
    ...
    <Card className="shadow-lg border-0">
      <CardHeader>
        <div className="flex items-center justify-between space-y-4 sm:space-y-0">
          <CardTitle className="text-xl font-bold">Scottish Water Forecast - REGRESSION</CardTitle>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-800">
                Forecast generated using a REGRESSION model trained on historical Scottish Water data.
              </p>
            </div>
          </div>
        </div>
