...
import { fetchRegionLevels, fetchStationNames } from "@/lib/groundwater";
...
export function EnglandRegressionChart({ region }: { region: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/water-levels/groundwater-predictions/?region=${region}&model_type=REGRESSION`);
        const forecastData: ForecastEntry[] = await res.json();
        if (Array.isArray(hist) && Array.isArray(forecastData)) {
          ...
          if (forecastData.length) {
            const avg = forecastData.reduce((s, d) => s + d.predicted_value, 0) / forecastData.length;
            setAvgPrediction(avg);
            const tr = forecastData[forecastData.length - 1].predicted_value - forecastData[0].predicted_value;
            setTrend(tr);
          }
        }
      } catch {
    ...
    <Card className="shadow-lg border-0">
      <CardHeader>
        <div className="flex items-center justify-between space-y-4 sm:space-y-0">
          <CardTitle className="text-xl font-bold">England Forecast - Regression</CardTitle>
          <div className="flex items-center space-x-3 sm:ml-4">
            ...
            <Button variant={showUncertainty ? "default" : "outline"} size="sm" onClick={() => setShowUncertainty(!showUncertainty)}>
              Uncertainty
            </Button>
          </div>
        </div>
      ...
      <CardContent>
        <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-800">Forecast generated using a regression model.</p>
            </div>
          </div>
        </div>
        <div className="h-80 mb-6">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg">
                        ...
                        {d.predicted && (
                          <>
                            ...
                            {showUncertainty && (
                              <p className="text-gray-600 text-sm">Range: {d.lowerBound.toFixed(1)}% - {d.upperBound.toFixed(1)}%</p>
                            )}
                          </>
                        )}
                      </div>
              ...
              />
              {showUncertainty && <Area type="monotone" dataKey="upperBound" stroke="none" fill="#a855f7" fillOpacity={0.1} />}
              {showUncertainty && <Area type="monotone" dataKey="lowerBound" stroke="none" fill="#ffffff" fillOpacity={1} />}
              <Line type="monotone" dataKey="actual" stroke="#2563eb" strokeWidth={3} dot={{ r: 4 }} />
              <Line type="monotone" dataKey="predicted" stroke="#a855f7" strokeWidth={3} strokeDasharray="5 5" dot={{ r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <div className={`text-2xl font-bold ${trend > 0 ? "text-green-600" : "text-red-600"}`}>{trend > 0 ? "+" : ""}{trend.toFixed(1)}%</div>
          </div>
