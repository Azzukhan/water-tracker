...
export function SouthernWaterCurrent({ reservoir }: { reservoir: string }) {
    ...
    fetch(`${API_BASE}/api/water-levels/southernwater-reservoirs/?reservoir=${reservoir}`)
      ...
      .then((d: Entry[]) => {
        if (Array.isArray(d) && d.length > 0) {
          const latest = d[0]
          const prev = d[1] ?? d[0]
          const avg = d.reduce((a, b) => a + b.current_level, 0) / d.length
          const change = +(latest.current_level - prev.current_level).toFixed(2)
          const diffAvg = +(latest.current_level - avg).toFixed(2)
          setCurrentData({
            currentLevel: latest.current_level,
      ...
      <Card className="lg:col-span-2 shadow-lg border-0">
        <CardHeader className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-bold">Southern Water Reservoir Level</CardTitle>
