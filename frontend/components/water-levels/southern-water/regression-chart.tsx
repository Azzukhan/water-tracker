...
export function SouthernRegressionChart({ reservoir }: { reservoir: string }) {
    ...
    const fetchData = async () => {
      try {
        const [histRes, forecastRes, accRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/water-levels/southernwater/${reservoir}/REGRESSION`),
          fetch(`${API_BASE}/api/water-levels/southernwater-prediction-accuracy/?reservoir=${reservoir}&model_type=REGRESSION`),
        ]);
        ...
        if (Array.isArray(histData) && Array.isArray(rawForecastData)) {
          ...
          const forecastData = lastActualDate
            ? (() => {
                ...
                const minStartIdx = after.findIndex(f =>
                  (new Date(f.date).getTime() - new Date(lastActualDate).getTime()) / (1000 * 60 * 60 * 24) >= 7
                );
                if (minStartIdx === -1) return [];
                return after.slice(minStartIdx, minStartIdx + 4);
              })()
            : [];
    ...
    <Card className="shadow-lg border-0">
      <CardHeader>
        <div className="flex items-center justify-between space-y-4 sm:space-y-0">
          <CardTitle className="text-xl font-bold">Southern Water Forecast - Regression</CardTitle>
