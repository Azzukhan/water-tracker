            ...
            if resp.status_code == 200 and resp.json():
                ...
                if stations.exists():
                    ...
                    results.append({
                        ...
                        'current': CurrentWeatherSerializer(current).data if current else None
                    })
...
class WeatherStationViewSet(viewsets.ModelViewSet):
    ...
    def current_weather(self, request, pk=None):
        ...
        weather = CurrentWeather.objects.filter(station=station).order_by('-timestamp').first()
        if weather:
            serializer = CurrentWeatherSerializer(weather)
            return Response(serializer.data)
        return Response({"detail": "No current weather data available for this station."}, status=404)
